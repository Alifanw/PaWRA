...
export default function ReportDateExport({ filters = {}, onFilterChange = () => {} }) {
    ...
    const onExport = (format = 'xlsx') => {
        // Use explicit admin paths to avoid depending on Ziggy's `route()` helper at module load time
        const base = '/admin/reports/export-all-xlsx';
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => onExport('xlsx')} className="inline-flex items-center px-3 py-2 rounded-md bg-emerald-600 text-white">Export .xlsx</button>
            <a href={`/admin/reports/export-all?start_date=${encodeURIComponent(start)}&end_date=${encodeURIComponent(end)}`} target="_blank" rel="noopener" className="inline-flex items-center px-3 py-2 rounded-md bg-slate-700 text-white">Export CSV</a>
        </div>
    );
}
